...
fn dec_to_snafu(d: i128, only_twos_in_dec: &Vec<i128>) -> String {
    ...
    if d > POWERS_OF_5[exp] + only_twos_in_dec[exp - 1] { // d > 1222, should return 2_____ with negative in underscore (need to subtract from 20000)
        ...
        let remain = d - 2 * POWERS_OF_5[exp]; // negative number
        if remain <= 0 {
        ...
        let mut res = "1".to_string();
        let remain = d - POWERS_OF_5[exp];
        if remain <= 0 {  // should return 1____ with negative in underscore (need to subtract from 10000)
            let minus_remain_in_snafu = dec_to_snafu(-remain, only_twos_in_dec);
            let num_of_chars = minus_remain_in_snafu.chars().into_iter().collect::<Vec<char>>().len();
            for _ in 0..exp - num_of_chars {
                res.push('0');
            ...
            res
        } else { // should return 1____ with positive in underscore (need to add to 10000)
            let minus_remain_in_snafu = dec_to_snafu(remain, only_twos_in_dec);
            let num_of_chars = minus_remain_in_snafu.chars().into_iter().collect::<Vec<char>>().len();
            for _ in 0..exp - num_of_chars {
                res.push('0');
